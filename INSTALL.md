...
4. Start the server:
   ...
   uv run python -m windows_operations_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn windows_operations_mcp.server:app --port 10748
   ```
5. Open `http://localhost:10748` or the frontend URL.
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/windows-operations-mcp/issues) |
