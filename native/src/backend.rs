...
const BACKEND_NAME: &str = "winops-mcp-backend.exe";
const BACKEND_PORT: u16 = 10748;
...
fn dev_backend_path() -> Option<PathBuf> {
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("winops-mcp-backend-x86_64-pc-windows-msvc.exe");
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app
        .path()
        .resolve("resources/winops-mcp-backend.exe", BaseDirectory::Resource)
    {
        tried.push(path.display().to_string());
        if path.exists() {
            return Ok(path);
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    command
        .current_dir(&workdir)
        .env("WINOPS_MCP_PORT", BACKEND_PORT.to_string())
        .env("WINOPS_MCP_HOST", "127.0.0.1")
        .env("WINOPS_TAURI", "1")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
