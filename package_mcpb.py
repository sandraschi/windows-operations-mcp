...
def get_file_list(directory: str) -> list[str]:
    ...
    try:
        ...
        print(f"Error listing files in {directory}: {e!s}")
        return []
...
def create_dxt_package(source_dir: str, output_file: str) -> bool:
    ...
    if os.path.exists(output_file):
        try:
            ...
            print(f"❌ Error removing existing file: {e!s}")
            return False
    ...
    try:
        with zipfile.ZipFile(output_file, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for i, file_path in enumerate(files_to_include, 1):
                try:
                    ...
                    print(f"⚠️  Warning: Could not add {file_path}: {e!s} (This file will be skipped)")
        ...
        print(f"\n❌ Error creating DXT package: {e!s}")
