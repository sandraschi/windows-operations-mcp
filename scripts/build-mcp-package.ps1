...
Write-Host "🛠️  MCPB Package Builder for Windows Operations MCP" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "🔍 Validating MCPB manifest..." -ForegroundColor Blue
    try {
    ...
    Write-Host "🏗️  Building MCPB package..." -ForegroundColor Blue
    try {
        ...
        Write-Host "✅ Package built: $packagePath" -ForegroundColor Green
    ...
    if (-not $NoSign) {
        Write-Host "🔐 Signing MCPB package..." -ForegroundColor Blue
        try {
            ...
            Write-Host "ℹ️  Package signing skipped (requires signing infrastructure)" -ForegroundColor Yellow
        ...
        Write-Host "⏭️  Package signing skipped (--NoSign specified)" -ForegroundColor Yellow
    }

    # Step 4: Verify the package
    Write-Host "🔍 Verifying MCPB package..." -ForegroundColor Blue
    try {
    ...
    try {
        ...
        Write-Host "   📋 To install: Drag the .mcpb file to Claude Desktop" -ForegroundColor Gray
    } catch {
        Write-Warning "Could not get package information: $_"
    }

    Write-Host ""
    Write-Host "🎉 MCPB package build completed successfully!" -ForegroundColor Green
    Write-Host ""
