...
function ToolCard({ tool, idx }: { tool: string; idx: number }) {
    ...
    const Icon = toolIcons[Object.keys(toolIcons).find(k => tool.includes(k)) || ''] || Box;
    ...
    return (
        ...
        >
            <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center transition-transform group-hover:scale-110 group-hover:rotate-6 duration-500">
                <Icon className={cn("w-7 h-7 text-muted-foreground transition-colors group-hover:text-primary")} />
            </div>
            
            <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                    <h3 className="text-xl font-black tracking-tighter italic">{tool}</h3>
                    <Badge variant="secondary" className="bg-white/5 border-white/5 text-[10px] font-bold tracking-widest uppercase">MCP TOOL</Badge>
                </div>
                <div className="flex items-center gap-4">
                    <p className="text-xs text-muted-foreground/60 font-medium tracking-tight">Authoritative Bridge Integration</p>
                    <div className="w-1 h-1 rounded-full bg-muted/30" />
                    <p className="text-[10px] font-mono text-primary/40 uppercase tracking-widest">windows_operations_mcp.core</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4">
                <div className="flex -space-x-2">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="w-6 h-6 rounded-full border-2 border-background bg-muted-foreground/10 flex items-center justify-center text-[8px] font-bold text-muted-foreground">IO</div>
                    ))}
                </div>
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Capabilities Inventory</span>
                    </div>
            ...
            <div className="grid gap-4">
                ...
                {(!tools?.tools || tools.tools.length === 0) && !isLoading && (
                    ...
                    >
                        ...
                        <h3 className="text-2xl font-black tracking-tighter mb-2 italic">Zero <span className="not-italic text-muted-foreground">Signals</span></h3>
                        <p className="text-sm text-muted-foreground max-w-xs text-center leading-relaxed">No authoritative tools were discovered on the current bridge. Check connection status.</p>
                ...
                {isLoading && (
                    <div className="space-y-4">
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className="h-28 glass-card rounded-[2rem] animate-pulse" />
                        ))}
                    </div>
                )}
            </div>
