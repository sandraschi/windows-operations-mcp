...
@tool(
    ...
    parameters={
        ...
        "include_system": {"type": "boolean", "description": "Include system processes", "default": False},
        "max_processes": {"type": "integer", "description": "Maximum number of processes to return", "default": 100},
    },
    required=[],
    returns={
        "type": "object",
...
def get_process_list(
    filter_name: str | None = None, include_system: bool = False, max_processes: int = DEFAULT_MAX_PROCESSES
) -> dict[str, Any]:
    """Get list of running processes with filtering options."""
    logger.info(
        "get_process_list_started", filter_name=filter_name, include_system=include_system, max_processes=max_processes
    )

    is_valid, error_msg = _validate_process_inputs(
    ...
    try:
        ...
        logger.info(
            "get_process_list_completed", process_count=len(processes), execution_time_ms=round(execution_time, 2)
        )
        return result
...
@tool(
    ...
    parameters={"pid": {"type": "integer", "description": "Process ID to get information for"}},
    required=["pid"],
    returns={
        "type": "object",
        "properties": {"success": {"type": "boolean"}, "process": {"type": "object"}, "error": {"type": "string"}},
    },
)
            ...
            try:
                ...
                process_info.update(
                    {
                        ...
                        "threads": [
                            {"id": t.id, "user_time": t.user_time, "system_time": t.system_time} for t in proc.threads()
                        ]
                        if proc.threads()
                        else [],
                    }
                ...
                try:
                    ...
                    process_info["open_files"] = (
                        [{"path": f.path, "fd": f.fd} for f in open_files[:50]] if open_files else []
                    )
                except (AccessDenied, AttributeError):
...
@tool(
    ...
    returns={
        "type": "object",
        "properties": {"success": {"type": "boolean"}, "resources": {"type": "object"}, "error": {"type": "string"}},
    },
)
def get_system_resources() -> dict[str, Any]:
    ...
    try:
        ...
        cpu_info = {
            ...
            "times": {"user": cpu_times.user, "system": cpu_times.system, "idle": cpu_times.idle},
        }
