...
async def windows_accounts(
    action: Literal["list_users", "add_user", "remove_user", "set_password", "list_groups", "manage_group"],
    ...
    try:
        ...
        if action == "manage_group":
            ...
            return {"success": True, "action": action, "data": {"status": f"User '{user}' {group_action}ed to group '{group}'"}}
        ...
        if ctx:
            ctx.error(error_msg)
            try:
                advice = await ctx.sample(f"Windows local account operation '{action}' failed. Error: {e}. Suggest repair.", max_tokens=100)
