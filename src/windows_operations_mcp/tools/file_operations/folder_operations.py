...
def create_directory_safe(directory_path: str, create_parents: bool = True, 
...
def delete_directory_safe(directory_path: str, recursive: bool = False, 
                        require_empty: bool = True) -> Dict[str, Any]:
    """
    ...
    def _delete_directory():
        ...
        if require_empty and any(dir_path.iterdir()):
            raise FileOperationError(
                f"Directory is not empty and require_empty=True: {directory_path}"
            )
        ...
        try:
            ...
            raise FileOperationError(f"Failed to delete directory: {str(e)}")
            
    return _delete_directory()

def move_directory_safe(source_path: str, destination_path: str, 
    ...
    def _move_directory():
        ...
        if dst.exists():
            if not overwrite:
                raise FileOperationError(
                    f"Destination already exists and overwrite=False: {destination_path}"
                )
            if not dst.is_dir():
                raise FileOperationError(
        ...
        try:
            ...
            raise FileOperation_error(f"Failed to move directory: {str(e)}")
            
    return _move_directory()

def copy_directory_safe(source_path: str, destination_path: str, 
                      overwrite: bool = False) -> Dict[str, Any]:
    """
