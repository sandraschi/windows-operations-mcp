...
def get_file_dates(file_path: str | Path) -> dict[str, datetime.datetime]:
    """
    ...
    if os.name == "nt":
        ...
        try:
            ...
            if not _GetFileTime(handle, ctypes.byref(created), ctypes.byref(accessed), ctypes.byref(modified)):
...
def set_file_dates(
    ...
    if os.name == "nt":
        ...
        try:
            ...
            if not _GetFileTime(
                handle, ctypes.byref(current_created), ctypes.byref(current_accessed), ctypes.byref(current_modified)
            ):
            ...
            if not _SetFileTime(handle, ctypes.byref(created_ft), ctypes.byref(accessed_ft), ctypes.byref(modified_ft)):
...
def set_file_dates_tool(
    file_path: str, created: str | None = None, modified: str | None = None, accessed: str | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        # Set the dates
        dates = set_file_dates(file_path, created=created_dt, modified=modified_dt, accessed=accessed_dt)
