...
@tool(
    ...
    returns={
        "type": "object",
        "properties": {"path": {"type": "string"}, "size": {"type": "integer"}, "created": {"type": "boolean"}},
    },
)
@log_operation("write_file")
@handle_operation("write_file", must_exist=False)
def write_file(path: str, content: str = "", overwrite: bool = False, encoding: str = "utf-8") -> dict[str, Any]:
    """
    ...
    return {"path": str(path), "size": len(content), "created": True}
...
@log_operation("read_file")
@handle_operation("read_file", must_exist=True)
def read_file(path: str, encoding: str = "utf-8", max_size: int | None = None) -> dict[str, Any]:
    """
    ...
    return {"path": str(path), "content": content, "size": len(content), "encoding": encoding}


@log_operation("delete_file")
@handle_operation("delete_file", must_exist=True)
...
@handle_operation("move_file")  # Will manually validate source in tool body if needed, or update decorator
def move_file(source: str, destination: str, overwrite: bool = False) -> dict[str, Any]:
    ...
    return {"source": str(source_path), "destination": str(dest_path), "moved": True}


@log_operation("copy_file")
@handle_operation("copy_file")
def copy_file(source: str, destination: str, overwrite: bool = False) -> dict[str, Any]:
    ...
    return {"source": str(source_path), "destination": str(dest_path), "copied": True}
