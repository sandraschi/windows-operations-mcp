...
@tool(
    ...
    parameters={
        "archive_path": {"type": "string", "description": "Path where the archive will be created"},
        "source_paths": {
            "type": "array",
            "items": {"type": "string"},
...
@tool(
    ...
    parameters={
        ...
        "extract_dir": {"type": "string", "description": "Directory where files will be extracted"},
        "members": {
            "type": "array",
            "items": {"type": "string"},
...
def extract_archive(archive_path: str, extract_dir: str, members: list[str] | None = None) -> dict[str, Any]:
    """
...
@tool(
    ...
    parameters={"archive_path": {"type": "string", "description": "Path to the archive file"}},
...
def _create_zip(
    archive_path: str, source_paths: list[str], compression_level: int, exclude_patterns: list[str]
) -> dict[str, Any]:
                ...
                if os.path.isdir(source_path):
                    for root, dirs, files in os.walk(source_path):
                        ...
                        dirs[:] = [
                            d
                            for d in dirs
                            if not _should_exclude_path(os.path.join(root, d), exclude_patterns, source_path)
                        ]
