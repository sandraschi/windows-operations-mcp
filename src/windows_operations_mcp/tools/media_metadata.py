...
@tool(
    ...
    parameters={"image_path": {"type": "string", "description": "Path to the image file"}},
)
    ...
    try:
        with Image.open(image_path) as img:
            ...
            return {
                "success": True,
                "metadata": exif_data,
                "file_info": {"format": img.format, "mode": img.mode, "size": img.size},
            }
...
def get_mp3_metadata(mp3_path: str | Path) -> dict[str, Any]:
    """
...
def get_media_metadata(file_path: str | Path) -> dict[str, Any]:
    """
