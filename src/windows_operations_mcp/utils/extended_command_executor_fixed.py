    ...
    def execute_powershell_advanced(
        ...
        **kwargs
        ...
        Args:
            ...
            working_directory: Working directory for the script
            timeout_seconds: Maximum execution time
            capture_output: Whether to capture output
            as_admin: Run with elevated privileges (Windows only)
            output_encoding: Output encoding
            convert_json: Automatically parse JSON output
            **kwargs: Additional arguments for execute_async
            
        Returns:
            CommandResult with execution details
        """
        # Create a temporary script file for complex scripts
        with tempfile.NamedTemporaryFile(suffix='.ps1', delete=False, mode='w', encoding='utf-8') as temp_script:
            try:
                # Write the script to the temporary file
                temp_script.write(script)
                temp_script_path = temp_script.name
            except Exception as e:
                logger.error(f"Failed to write temporary script: {e}")
                return CommandResult(
                    success=False,
                    exit_code=-1,
                    stdout='',
                    stderr=f'Failed to write temporary script: {str(e)}',
                    execution_time=0,
                    command=script,
                    working_directory=working_directory or os.getcwd(),
                    error=f'Failed to write temporary script: {str(e)}'
                )
